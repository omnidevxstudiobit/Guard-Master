...
- **Curtain** (`initCurtain`) — fence-vector draw-on loader, lifts on ready. Must never be able to stick: any JS boot error on a page keeps it up, so page modules must not throw at top level (see the panels `sel.sz` regression in [page-panels.md](page-panels.md)). Belt-and-braces: every curtain page carries an inline non-module watchdog `<script>` that lifts the curtain after 4 s even if page JS never boots (stalled data-layer fetch, pre-es2022 browser).
