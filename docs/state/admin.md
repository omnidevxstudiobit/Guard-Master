...
**Products — everything, per product** (`#/product/<id>`), rebuilt to **Shopify's product-editor anatomy** (researched against Shopify's admin, adapted honestly): edits batch into a working copy behind a sticky **"Unsaved changes" Save/Discard bar** (nothing commits until Save; Discard reverts). Main column: Title/Description card → **Media** (grid with large LEAD tile, HOVER badge, hover tools ◀▶✕, add from pool/by path) → **Pricing** (ZAR + "customers see" USD; deliberately *no compare-at* — the site never fakes discounts) → **Inventory** (read-only SKU pattern + lead-time note; deliberately *no quantity tracking* — site honesty rule) → **Variants** (option sets with values as removable tag pills + Enter-to-add; labels are pricing keys, unresolvable combos fall back to "From price · confirmed on order") → **Search engine listing** (read-only Google-style preview generated from title/description). Sidebar: **Status** (Active/Hidden) → **Publishing** (live checklist: home grid, catalogue, rail, each collection page) → **Product organization** (product type = group `clearview/razor/access`, collections membership, tags) → **"You may also like"** pairs → reset-to-factory. Saving also diffs collections membership into `collections.<id>.products`. Plus, on the list screen: **catalogue ordering** (▲▼ → `settings.productOrder`) and the **best-sellers rail** order (`settings.bestSellers`).
**Discounts & gift cards — Shopify anatomy** (web-researched): create flow = Method (code + Generate) → Type (percentage / fixed ZAR amount) → Value → **Minimum purchase** → **Active dates** (start/end); codes table shows type/value, minimum, date window, **derived status** (active/scheduled/expired/off/redeemed) and a this-browser Used count. **Gift cards**: issue with value/recipient/expiry, full code revealed once, list **masked to last 4** (Copy retrieves it), Mark-redeemed + delete. Checkout enforces minimum + date window **with the reason spelled out** ("only starts on…", "expired on…", "needs a minimum order of $…", "already been redeemed") and re-checks the applied code at submit. Deliberately **no usage limits** — unenforceable without a backend, and the UI says so.
Also: collections, homepage cover (real-photo pool picker), promo bar, support email, ZAR→USD display rate, checkout fulfilment toggles + note (never both off), draft orders (open into the real checkout), metafields, media uploads (≤2.5 MB, publish commits them), pages & blog, both dot-map city sets, webhook subscriptions (orders/create, orders/fulfilled, *), publish/export/discard.
